...
La première difficulté rencontrée durant ce projet a été la mise en place d'un algorithme de jeu. Le problème principal était qu'il fallait bien réfléchir à quand faire une vérification de la présence de moulin, comment l'automatiser et de manière générale dans quel ordre il fallait permettre aux joueurs de faire des actions. La solution a donc été de créer deux fonctions principales: une qui s'occupe de la sélection des cases et l'autre de la sélection des pions, et de les faire communiquer par le biais de variables globales.

La deuxième difficulté a été de concilier les parties en ligne et les parties en local. Le risque était constamment qu'une des fonctions appelée ne soit pas définie, car cherchée dans le mauvais fichier, ou qu'une fonction indésirée s'exécute. La solution a donc été de faire de nombreux copier-coller des fonctions des parties en local dans un deuxième fichier spécifique aux parties en ligne et les adaptant légèrement. Cette pratique n'est certes pas optimale, mais permet de régler ce problème.

La dernière difficulté a été la mise en place des chronomètres pour la version en ligne. Après quelques tests, j'ai constaté que la solution qui fonctionnait le mieux était de décrémenter les variables de temps côté serveur (de la même manière que dans le code pour les parties en local), mais d'avoir également une décrémentation du temps chez les clients. Le temps restant des joueurs est sauvegardé dans l'instance de la classe <em>PartieMoulin</em> de la partie en cours. Lorsque c'est au tour d'un des joueurs de déplacer ses pions, son temps de jeu restant est envoyé aux deux clients. Chez ceux-ci, une fonction, <em>timerLigne(temps, joueur)</em>, est appelée. Cette dernière contient un <em>setInterval()</em>, qui permet de décrémenter le temps sur les pages HTML chaque seconde. Il y a donc trois intervalles synchronisés en même temps: un chez le serveur qui décrémente directement les variables de temps restant des joueurs et un chez chacun des clients qui modifie l'affichage des temps sur les pages HTML.
