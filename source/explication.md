...
La dernière difficulté a été la mise en place des chronomètres pour la version en ligne. Après quelques tests, j'ai constaté que la solution qui fonctionnait le mieux était de décrémenter les variables de temps côté serveur, de la même manière que dans le code pour les parties en local, mais d'avoir également une décrémentation du temps chez les clients. Le temps restant des joueurs est sauvegardée dans l'instance de la classe <em>PartieMoulin</em> de la partie en cours. Lorsque c'est au tour d'un des joueurs de déplacer ses pions, ce temps restant est envoyé aux deux clients. Chez ceux-ci, une fonction <em>timer(temps)</em> 
